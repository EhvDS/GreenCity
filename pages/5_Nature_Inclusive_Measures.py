...
# Display selected item details above the grid
...
st.info("The selected item data will be loaded above the grid with all the items.")
st.markdown("""
...
for i in range(0, len(filtered_items), 4):  # Loop through items with a step of 4 (one row per loop)
    ...
    for j, item in enumerate(filtered_items[i:i+4]):  # Populate the row with up to 4 items
        with cols[j]:
            ...
            st.button(item["name"], key=item["name"] + "_grid")
